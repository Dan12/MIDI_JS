    ...
    MIDI_Workspace.prototype.generalInput = function(e){
        ...
        if(e.detail.mouseDown){
            ...
            if(e.detail.mouseY > this.heightOffset && e.detail.mouseY < this.heightOffset+scrubBarHeight)
                this.scrubBarAt = Math.round(Math.max(0,(e.detail.mouseX-this.horizontalOffset-keySize)/this.BeatsToPixels)*4/this.BeatsPerSection)/(4/this.BeatsPerSection);
    ...
    MIDI_Workspace.prototype.buttonPress = function(b){
        if(b.text == "Record"){
            ...
            if(this.isPlaying){
                this.isPlaying = false;
                this.header.resetButtons();
                this.noteHandler.stopPlaying(false);
                b.isDown = true;
            }
        }
        else if(b.text == "Stop"){
